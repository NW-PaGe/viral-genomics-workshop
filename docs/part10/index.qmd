...
*Imagine this tree to represent viral sequences of pathogen X. This phylogenetic tree uses a a character-based approach to phylogenetic construction called Maximum Likelihood. In other words, this Maximum Likelihood tree represents a tree that best explains the observed patterns of mutations. In the tree, we see that SeqA and the Reference sequence are on the same branch and stacked vertically, indicating that the sequences are genetic identical. Starting from Seq A, the tree shows that Seq B is the sequence that branches off, but is on the same node that SeqC and SeqD branch off from. SeqB shows a divergence of approximately 0.052, indicating 1 mutation difference from the reference (sequence length: 19 x divergence: 0.052 = ~1). Seq D branches off next; although it has two mutations relative to the reference, it shares key differences with SeqE. The most closely related pairs in the dataset is the Ref and SeqA,which differ by zero mutations and are genetically identical; as well as Seq D and SeqE, which differ from each other by just a single mutation and share additional differences not found in the other sequences. This pattern suggests they may share a recent common ancestor and in terms of epidemiological context this may represent part of a localized transmission cluster.* 
...
![Example illustrating a phylogenetic tree](../images/ex_tree.jpeg)
