...
Imagine this tree to represent viral sequences of pathogen X. This phylogenetic tree was constructed using a character-based method called Maximum Likelihood. Rather than simply counting differences between sequences, this approach infers the tree that best explains the observed pattern of mutations across the entire alignment. The tree is rooted using the reference genome (Ref). Rooting trees can help give directionality to the **topology** of the tree. The Ref and SeqA are genetically identical, and both appear stacked on the same branch, indicating no differences between them.
...
&nbsp;&nbsp;&nbsp; - **Evolutionary modeling**: Character-based trees incorporate substitution models (like GTR) to estimate mutation rates and account for multiple changes at the same site.  
