...
A phylogenetic tree is a branching diagram that represents the evolutionary relationships between genome sequences, based on patterns observed in their genetic differences. In this outbreak investigation, a tree that reflects evolutionary history can help determine how closely related viral genomes are across different individuals and species. Think of it like a family tree for the virus.
...
This tree represents the evolutionary relationships between viral genome sequences of pathogen X, inferred using a character-based method such as Maximum Likelihood. Rather than just counting mutations, this method analyzes patterns of change across the entire alignment to infer the tree that best explains the data.

The tree is **rooted at the reference genome (Ref)**, meaning all other sequences are interpreted relative to it. **SeqA** is genetically identical to **Ref**, indicated by both appearing at a divergence of 0.00.

From that shared point, **SeqB** branches off next, with a divergence of about 0.06, suggesting it has accumulated some genetic differences since splitting from the reference.

**SeqC** and **SeqD** form a small cluster—they share a common ancestor and branch from the same internal node. SeqC diverges to ~0.10, and SeqD slightly further to ~0.12, indicating it has acquired more mutations than SeqC.
...
- **SeqC and SeqD** are closely related, forming a cluster.  
...
The tree’s structure helps us visualize how the virus may have evolved and spread—and which sequences are more closely related in time and ancestry.

---

### Divergence

In this tree, the horizontal axis represents **divergence**, the estimated amount of genetic change that has occurred from the root (ancestral sequence) to each sample. This tells us how much each sequence has evolved over time.
...
1. **Speed vs. accuracy**  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Distance-based trees are faster. Character-based methods are slower but more accurate for closely related sequences.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; - **Homoplasy** is when the same mutation appears in different sequences independently—not due to shared ancestry.  
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Character-based trees use substitution models (like GTR) to account for the likelihood and direction of changes between nucleotides.
