...
You will explore a fictional scenario that unfolds like a detective story. It is set in an *idealized world* -- one where the importance of genomics has already been recognized; and the infrastructure is in place. The tools are available, the workforce is trained, and the systems are ready; the only task that remains is to *respond*. 

This simplification is intentional; it allows us to focus on the logic, decision points, and workflows that shape effective outbreak response using next-generation sequencing technology and genomic surveillance. As you progress, you will encounter:
